...
namespace ApiClient
{
	public class EodClient : IEodClient
	{
		...
		public EodOutstandingShares GetBalanceSheet(string ticker)
		{
			var request = $"{route}/fundamentals/{ticker}?filter=Balance_Sheet&{jsonFormatting}" + GetApiKeyRequestPhrase();
			var response = MakeRequest<EodOutstandingShares>(request);
		...
		public EodOutstandingShares GetCashFlow(string ticker)
		{
			var request = $"{route}/fundamentals/{ticker}?filter=Cash_Flow&{jsonFormatting}" + GetApiKeyRequestPhrase();
			var response = MakeRequest<EodOutstandingShares>(request);
		...
		public EodOutstandingShares GetIncomeStatement(string ticker)
		{
			var request = $"{route}/fundamentals/{ticker}?filter=Income_Statement&{jsonFormatting}" + GetApiKeyRequestPhrase();
			var response = MakeRequest<EodOutstandingShares>(request);
