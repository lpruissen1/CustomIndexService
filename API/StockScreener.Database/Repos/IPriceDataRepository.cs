...
namespace StockScreener.Database.Repos
{
    public interface IPriceDataRepository : IBaseRepository<PriceData>
    {
		...
		IEnumerable<TPriceEntry> GetMany<TPriceEntry>(IEnumerable<string> tickers) where TPriceEntry : PriceData;
		...
		IEnumerable<Candle> GetPriceData<TPriceEntry>(string ticker) where TPriceEntry : PriceData;
    }
}
